...
## Task 10 — 데이터 갱신 (2026-09-06)
- [x] scan-folders.js 리포트 대조 — 마지막 등록(2026-07-19) 이후 미등록 폴더 4건 확인
- [x] Scroll World 시네마틱·중장기 종합계획 정책동향·키움 REST API 트레이딩·NEPSA 워크벤치 추가 (78 → 82)
- [x] build-data.js 재생성 + 인라인 배열 82건 json 일치 검증, http 서빙으로 82행 렌더·콘솔 에러 0 확인
- 4건 모두 배포 URL 미확인 → url 필드 비움. 배포처 확인되면 채울 것
- projects.json은 **날짜순 정렬이 아니라 추가순**이다. 정렬하면 diff가 전체로 번지니 끝에 append할 것
